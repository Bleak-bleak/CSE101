...
def processInstruction(instruction):
    ...
    current = hexToBin(instruction)
    if len(hexToBin(instruction)) != 24:
        return "ERROR"
    else:
        opcode = current[:4]
        address = current[5:8]
        ...
        if opcode == "0000":
            ...
            if address == "000":
                accumulator = operand
                return "LOAD "+ str(operand) +" INTO ACCUNMULATOR"
            elif address == "001":
                if operand in ram:
            ...
            if address == "000":
                ...
                if operand in ram:
                    accumulator -= int(ram[operand])
                    return "SUBTRACTED CONTENT OF ADRESS " + str(operand) + " FROM ACCUMULATOR"
                else:
                    return "ERROR"
            else:
                return "ERROR"
            ...
            if address =="000":
                ...
                if operand in ram:
                    accumulator = int(ram[operand])*accumulator
            ...
            if address =="000":
                ...
                if operand in ram:
                    aacumulator = accumulator//int(operand)
